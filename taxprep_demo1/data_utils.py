...
def load_or_generate_sample(n=200):
    ...
    df = pd.DataFrame({
        ...
        'service_type': np.random.choice(['Individual','Business','Consulting'], size=n),
        'turnaround_time_days': np.random.poisson(7, size=n) + np.random.randint(0,10,size=n),
        ...
        'last_feedback_text': np.random.choice([\"Great service\",\"Late delivery\",\"Incorrect forms\",\"Helpful advisor\"], size=n)
    })
    return df
